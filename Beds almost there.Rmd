...
```{r}
avg_beds <- wranged_beds_no_gj %>% 
  filter(specialty_name %in% c("All Acute", "All Specialties"),
         hb %in% "Scotland") %>%
  group_by(date) %>% 
  summarise(avg = mean(average_occupied_beds)) 
  ...
  geom_point() +
  geom_line() +
  geom_line(aes(x = avg_beds$date, y = avg_beds$avg), col = "red", size = 1.5)
```
...
activity_no_gj %>%  
  filter(hb %in% "Scotland",
         ...
         specialty_name %!in% c("Clinical Genetics", 
